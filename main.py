...
async def lifespan(app: "FastAPI"):
    ...
    if missing:
        raise RuntimeError(
            f"Missing pickle file(s) next to main.py: {', '.join(missing)}. "
            f"Train/export your model first, or check BASE_DIR."
        )
...
# browsers reject wildcard-origin + credentials combos per the CORS spec.
# Since this frontend doesn't send cookies/auth headers, False is correct here.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def tmdb_get(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.exception("TMDB Request Error")
...
async def tmdb_search_movies(query: str, page: int = 1) -> Dict[str, Any]:
    ...
    Frontend uses this for suggestions and grid.
    """
